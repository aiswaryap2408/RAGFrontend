...
import { getReportPrompt, updateReportPrompt, getReportPromptHistory } from '../../api';

const ReportPromptEditor = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const [promptRes, historyRes] = await Promise.all([
                getReportPrompt(),
                getReportPromptHistory()
            ]);
            setPrompt(promptRes.data.prompt);
            setHistory(historyRes.data.history || []);
        } catch (err) {
            setMessage({ type: 'error', text: 'Failed to load report prompt data' });
            console.error(err);
        } finally {
            setLoading(false);
        }
    ...
    const handleSave = async () => {
        ...
        try {
            await updateReportPrompt(prompt);
            setMessage({ type: 'success', text: 'Report prompt updated successfully!' });
            // Refresh history
            const historyRes = await getReportPromptHistory();
            setHistory(historyRes.data.history || []);
        } catch (err) {
            setMessage({ type: 'error', text: 'Failed to update report prompt' });
            console.error(err);
        } finally {
            setSaving(false);
        }
    ...
    return (
        <div className="bg-slate-900/50 rounded-3xl shadow-sm border border-slate-800/60 overflow-hidden">
            <div className="p-6 border-b border-slate-800/60 flex justify-between items-center">
                <div>
                    <h3 className="text-xl font-black text-white tracking-tight">Detailed Report Prompt Configuration</h3>
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">
                        Defines behavior for generating long-form astrology reports
                    </p>
                </div>
                ...
                {activeView === 'editor' ? (
                    <>
                        <textarea
                            ...
                            placeholder="Enter report prompt here..."
                        />
