...
import { getPsycologyPrompt, updatePsycologyPrompt, getPsycologyPromptHistory } from '../../api';

const PsycologyPromptEditor = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const [promptRes, historyRes] = await Promise.all([
                getPsycologyPrompt(),
                getPsycologyPromptHistory()
            ]);
            setPrompt(promptRes.data.prompt);
            setHistory(historyRes.data.history || []);
        } catch (err) {
            setMessage({ type: 'error', text: 'Failed to load psychology prompt data' });
            console.error(err);
        } finally {
            setLoading(false);
        }
    ...
    const handleSave = async () => {
        ...
        try {
            await updatePsycologyPrompt(prompt);
            setMessage({ type: 'success', text: 'Psychology prompt updated successfully!' });
            // Refresh history
            const historyRes = await getPsycologyPromptHistory();
            setHistory(historyRes.data.history || []);
        } catch (err) {
            setMessage({ type: 'error', text: 'Failed to update psychology prompt' });
            console.error(err);
        } finally {
            setSaving(false);
        }
    ...
    return (
        <div className="bg-slate-900/50 rounded-3xl shadow-sm border border-slate-800/60 overflow-hidden">
            <div className="p-6 border-b border-slate-800/60 flex justify-between items-center">
                <div>
                    <h3 className="text-xl font-black text-white tracking-tight">User Psychology Prompt Configuration</h3>
                ...
                {activeView === 'editor' ? (
                    <>
                        <textarea
                            ...
                            placeholder="Enter psychology prompt here..."
                        />
...
export default PsycologyPromptEditor;
